...
public class TableManager : MonoBehaviour
{
    ...
    {
        GameObject cell = GameObject.Instantiate(defaultCell, new Vector3(x * cellSpacing, -y * cellSpacing, 0.0f), Quaternion.identity, canvas.transform);
    }

    void CreateCell(int x, int y, int num, string name)
    {
        GameObject cell = GameObject.Instantiate(defaultCell, new Vector3(x * cellSpacing, -y * cellSpacing, 0.0f), Quaternion.identity, canvas.transform);
        ...
        for(int x = 0; x < TableInfo.x; x++)
        {
            for(int y = 0; y < TableInfo.y; y++)
            {
                ...
                switch(n)
                {
                    ...
                    default: //positives
                        CreateCell(x, y, n, TableInfo.elems[n - 1]);
                        break;
                }
            }
        }
